...
from lftools_ng.core.credential_manager import (
    CredentialManager, MigrationOptions, MigrationResult,
    Credential, CredentialType, CredentialScope, CredentialFilter
)
...
class ProjectAwareMigrationManager:
    ...
    def get_github_url_for_repository(self, repository_name: str, project: str = "O-RAN-SC") -> str:
        """
        Generate GitHub URL for a repository.

        Args:
            repository_name: The repository name (e.g., "aal-virt")
            project: The project name (defaults to "O-RAN-SC")
        ...
        for proj in projects:
            if proj.get("name") == project or project.upper() in proj.get("aliases", []):
                project_data = proj
                break
    ...
    def filter_repository_credentials(self, credentials: List[Credential], filter_pattern: str = "deployment") -> List[Credential]:
        ...
        for credential in credentials:
            # Check if credential name contains the filter pattern (case insensitive)
            if filter_pattern.lower() in credential.name.lower():
...
def main():
    ...
    app = typer.Typer(name="oran-migrate", help="Generic Repository Credential Migration Tool")
    ...
    def migrate_repository_credentials(
        ...
        project: str = typer.Option(
            "O-RAN-SC",
            "--project",
            help="Project name for repository URL generation"
        ),
        ...
        oran-migrate repository --jenkins-server https://jenkins.o-ran-sc.org --jenkins-user admin --jenkins-password token --filter-pattern "nexus deployment" --dry-run

        # Migrate all repository credentials with default pattern
        oran-migrate repository --jenkins-server https://jenkins.example.org --jenkins-user admin --jenkins-password token --project "MyProject"

        # Migrate ONAP credentials with custom pattern
        oran-migrate repository --jenkins-server https://jenkins.onap.org --jenkins-user admin --jenkins-password token --project "ONAP" --filter-pattern "artifact deployment"

        # Migrate a single credential
        oran-migrate repository --jenkins-server https://jenkins.o-ran-sc.org --jenkins-user admin --jenkins-password token --single aal-virt
        """
        console = Console()
    ...
    # Backward compatibility: nexus command for O-RAN-SC
    @app.command("nexus")
    def migrate_nexus_credentials_compat(
        # Jenkins connection options
        jenkins_server: str = typer.Option(
            ...,
            "--jenkins-server", "-s",
        ...
        single_credential: Optional[str] = typer.Option(
            None,
            "--single", "-1",
            help="Migrate only a single credential by repository name"
        ),
        project: str = typer.Option(
            "O-RAN-SC",
            "--project",
        ...
        """
        [DEPRECATED] Use 'repository' command instead.

        Migrate O-RAN-SC Nexus deployment credentials from Jenkins to 1Password.
        This is a backward-compatible command that calls the generic repository migration with O-RAN-SC defaults.
        """
        console = Console()
        console.print("[yellow]⚠️  DEPRECATED: The 'nexus' command is deprecated. Use 'repository --filter-pattern \"nexus deployment\"' instead.[/yellow]\n")

        # Call the generic repository migration with O-RAN-SC specific defaults
        return migrate_repository_credentials(
            ...
            project=project,
            filter_pattern="nexus deployment",  # O-RAN-SC specific
